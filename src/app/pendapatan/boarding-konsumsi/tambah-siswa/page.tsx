...
export default function TambahSiswaEkstra() {
  ...
  // Fungsi untuk mencari siswa (autocomplete)
      ...
      <div className="bg-white border rounded-md shadow-md p-6 max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4 relative">
            <label className="block mb-2 font-semibold">Cari Siswa (NISN/Nama)</label>
            <input
              id='cari_siswa'
              type="text"
              value={searchQuery}
            ...
            {showDropdown && searchResults.length > 0 && (
              <div 
                ref={dropdownRef}
                className="absolute z-10 w-full bg-white border border-gray-300 rounded shadow-lg max-h-60 overflow-y-auto"
              >
                {searchResults.map((siswa) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-600">{siswa.level} - {siswa.akademik}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          {selectedSiswa && (
            <div className="mb-4 bg-gray-50 p-3 rounded-md">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div><span className="text-gray-500">NISN:</span> {selectedSiswa.nisn}</div>
                <div><span className="text-gray-500">Nama:</span> {selectedSiswa.nama_siswa}</div>
                <div><span className="text-gray-500">Level:</span> {selectedSiswa.level}</div>
                <div><span className="text-gray-500">Akademik:</span> {selectedSiswa.akademik}</div>
              </div>
            </div>
          )}
